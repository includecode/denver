...
# -*- coding: utf-8 -*-
"""
Created on Tue Jun 16 04:32:09 2020
...
def invert_matrix(A, tol=None):
    ...
    print("\n\n\nOn a la matrice d'identité suivante:")
    print_matrix("Matrice d'identité", I)

    # Section 3: Perform row operations
    indices = list(range(n)) # to allow flexible row referencing ***
    for fd in range(n): # fd stands for focus diagonal
        fdScaler = 1.0 / AM[fd][fd]
        print("Facteur de multiplication de la ligne: 1 / ",AM[fd][fd], " = " , fdScaler)
        ...
        for i in indices[0:fd] + indices[fd+1:]: # *** skip row with fd in it.
            ...
            print("AM[{}][{}] = AM[{}][{}] - {} * AM[{}][{}]".format(i, j, i, j, crScaler, fd, j))
            for j in range(n): # cr - crScaler * fdRow, but one element at a time.
                AM[i][j] = AM[i][j] - crScaler * AM[fd][j]
                IM[i][j] = IM[i][j] - crScaler * IM[fd][j]
        print_matrix('Matrice courante', AM)
        print()
        print_matrix("Matrice d'identité", IM)
    return IM
